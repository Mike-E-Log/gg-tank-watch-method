...
- **[`failure-analysis.md`](failure-analysis.md)**: the 12-failure-mode (F1–F12) red-team.
  Every identified way the system could lie, fabricate, stale-date, or be injected, ranging
  from a fabricated all-clear (F1) to prompt injection via a scraped page (F12). Proves the
  failure surface was mapped adversarially, not assumed.
...
- **[`decision-authority.md`](decision-authority.md)**: the design + threat model, showing how
  the AI is scoped to situational awareness while the human keeps the final safety verdict.
